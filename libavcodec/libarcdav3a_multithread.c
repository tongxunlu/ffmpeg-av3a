...
typedef struct {
    AVS3DecoderOutput queue[MAX_FRAMES];
	AVPacket packetque[MAX_PKT];
	int pkthead;
    int pkttail;
    int head;
    int tail;
...
static av_cold int arcdav3a_decode_init(AVCodecContext *avctx)
{
    ...
    pthread_mutex_init(&h->framequeue.mutex, NULL);
	pthread_mutex_init(&h->framequeue.pktmutex, NULL);
	pthread_cond_init(&h->framequeue.cond, NULL);
	pthread_cond_init(&h->framequeue.pktcond, NULL);
	h->threadCreated = false;
    ...
    h->t1 = millisecondsSinceEpoch;*/
    return 0;
}

static av_cold int arcdav3a_decode_flush(AVCodecContext *avctx)
{
...
static int dav3a_decode_frame(AVCodecContext *avctx, const char * pIn, unsigned long lenIn, unsigned long* lenConsumed, AVS3DecoderOutput* pOut)
{
	...
	*lenConsumed = lenIn;
	...
	do {
		...
		if (ret != AVS3_TRUE)
		{
			av_log(avctx, AV_LOG_DEBUG, "parse_header return:%d, pos(%d), m_dwDataLen(%d)", ret, pos, h->m_dwDataLen);
			break;
		}
...
//		ASLOG_MsgA(5, "%s, m_dwDataLen:%d, m_hAvs3->bitsPerFrame:%d, %d", __FUNCTION__, m_dwDataLen, m_hAvs3->bitsPerFrame, m_hAvs3->bitsPerFrame/8);
		...
		int outlen = 0;
		av_log(avctx, AV_LOG_DEBUG, "begin  avs3_decode! pos:%d m_dwDataLen:%d h->m_pBuffer:%p pOut->pOutData:%p\n", pos, h->m_dwDataLen, h->m_pBuffer, pOut->pOutData);
...
//		if (pOut->pMeta && (pOut->nMeta > 0))
		...
		{
			dst[i] = outbuf[i] * MAX_SINT16;//float转int
		}
...
static  int decoder_thread(void *arg)
{
	...
	while(h->m_hAvs3)
	{
		FrameQueue* queue = &h->framequeue;
        pthread_mutex_lock(&queue->pktmutex);
		if(queue->pkthead == queue->pkttail) {
			// packet队列空，等待添加packet
			queue->pkthead = queue->pkttail = 0;
			memset(&queue->packetque[0], 0, sizeof(queue->packetque[0]));
			av_log(avctx, AV_LOG_DEBUG, "!!!pktqueue is empty! wait to add pkt! queue->pkthead==pkttail==%d\n", queue->pkthead);
			pthread_cond_wait(&queue->pktcond, &queue->pktmutex);
			av_log(avctx, AV_LOG_DEBUG, "decoder_thread:wait cond and mutex end!\n");
		}
		AVPacket avpkt = queue->packetque[queue->pkttail];
		queue->pkttail = (queue->pkttail + 1) % MAX_PKT;
		av_log(avctx, AV_LOG_DEBUG, "decoder_thread: set pktqueue.tail=%d\n", queue->pkttail);
		pthread_mutex_unlock(&queue->pktmutex);
		if (avpkt.size>0)
		{
			...
			while (size > 0)//
			{
				...
				int ret = dav3a_decode_frame(avctx, pbIn, size, &lenConsumed, &h->out_frame);
				if (ret < 0)
				...
				/*while ((queue.head + 1) % MAX_FRAMES == queue.tail) {
					// 队列满，等待输出线程取出帧
					av_log(avctx, AV_LOG_DEBUG, "queue is full wait render thread to render frame!\n");
					pthread_cond_wait(&queue.cond, &queue.mutex);
				}*/
				if((queue->head + 1) % MAX_FRAMES == queue->tail){
		            av_log(avctx, AV_LOG_DEBUG, "decode thread:  queue FULL %d\n", MAX_FRAMES);
	            }
				queue->queue[queue->head] = h->out_frame;
				queue->head = (queue->head + 1) % MAX_FRAMES;
				av_log(avctx, AV_LOG_DEBUG, "decoder_thread: set queue.head=%d\n", queue->head);
				pthread_mutex_unlock(&queue->mutex);

				// 通知输出线程有新的帧可用
				pthread_cond_signal(&queue->cond);
				av_log(avctx, AV_LOG_DEBUG, "send queue.cond!\n");
			}
			av_log(avctx, AV_LOG_DEBUG, "av_packet_unref!\n");
			av_packet_unref(&avpkt);
			av_log(avctx, AV_LOG_DEBUG, "end av_packet_unref!\n");
		}//end if (ret >= 0 && avpkt.size>0)
	}

    return 0;
}

static int arcdav3a_receive_frame(AVCodecContext *avctx, AVFrame *frm)
{
    av_log(avctx, AV_LOG_DEBUG, "begin arcdav3a_receive_frame! %p \n",avctx);
	arcdav3a_context *h = avctx->priv_data;
	int ret;
	...
	if (ret == AVERROR_EOF)
		return AVERROR_EOF;
	av_log(avctx, AV_LOG_DEBUG, "arcdav3a_receive_frame ff_decode_get_packet ret:%d avpkt.size:%d  avctx:%p\n",ret, avpkt.size, avctx);
	...
	if (num == 1)
	{
		...
		fpout = fopen("test.pcm", "w");
	}
    else
	{
        fpin = fopen("test.av3a", "a+");
		fpout = fopen("test.pcm", "a+");
	}
	...
	if (ret >= 0 && avpkt.size>0){
		FrameQueue* queue = &h->framequeue;
		pthread_mutex_lock(&queue->pktmutex);
		queue->packetque[queue->pkthead] = avpkt;
		queue->pkthead = (queue->pkthead + 1) % MAX_PKT;
		av_log(avctx, AV_LOG_DEBUG, "arcdav3a_receive_frame: set pktqueue.head =%d\n", queue->pkthead);
		if((queue->pkthead + 1) % MAX_PKT == queue->pkttail){
			av_log(avctx, AV_LOG_DEBUG, "arcdav3a_receive_frame:  pktqueue FULL %d\n", MAX_PKT);
		}
		pthread_mutex_unlock(&queue->pktmutex);
		// 通知解码线程有新的pkt可用
		pthread_cond_signal(&queue->pktcond);
	}
	else
	    return AVERROR(EAGAIN);
	
	if(h->threadCreated == false){
		ret = pthread_create(&h->thread, NULL, decoder_thread, avctx);
		if (ret) {
			ret = AVERROR(ret);
			av_log(avctx, AV_LOG_ERROR, "pthread_create() failed: %s\n",
					av_err2str(ret));
			return ret;
		}
		h->threadCreated = true;
	}

	//get frame and render if necessary
	while (h->m_hAvs3) {
        FrameQueue* queue = &h->framequeue;
		pthread_mutex_lock(&queue->mutex);
		av_log(avctx, AV_LOG_DEBUG, "arcdav3a_receive_frame: get queue.head =%d  queue.tail=%d\n", queue->head, queue->tail);
		if(queue->head == queue->tail) {
			// 队列空，等待解码线程添加帧
			queue->head = queue->tail = 0;
			memset(&queue->queue[0], 0, sizeof(queue->queue[0]));
			av_log(avctx, AV_LOG_DEBUG, "!!!queue is empty! wait decode thread to add frame\n");
			pthread_cond_wait(&queue->cond, &queue->mutex);
			av_log(avctx, AV_LOG_DEBUG, "arcdav3a_receive_frame: wait cond and mutex end!\n");
		}

		AVS3DecoderOutput out_frame = queue->queue[queue->tail];
		queue->tail = (queue->tail + 1) % MAX_FRAMES;
		av_log(avctx, AV_LOG_DEBUG, "arcdav3a_receive_frame: set queue.tail=%d\n", queue->tail);
		pthread_mutex_unlock(&queue->mutex);
		// 通知解码线程有空间可用
		//pthread_cond_signal(&queue->cond);
		if(h->m_bGotMD){
			ret = RenderPCM(avctx, &out_frame);
			if (ret < 0)
			return AVERROR(EAGAIN);


			frm->nb_samples = 1024;
			frm->sample_rate = out_frame.nSamplerate;
			frm->channels = out_frame.nChannel;
			frm->channel_layout = av_get_default_channel_layout(out_frame.nChannel);
			ChannelNumConfig chconf = (ChannelNumConfig)out_frame.nChCfg;
			if(out_frame.nChannel == 1)
				frm->ch_layout = (AVChannelLayout)AV_CHANNEL_LAYOUT_MONO;// need set frame->ch_payout to frame ch_out verification
			else if(out_frame.nChannel == 2)
				frm->ch_layout = (AVChannelLayout)AV_CHANNEL_LAYOUT_STEREO;// need set frame->ch_payout to frame ch_out verification
			else if(chconf == CHANNEL_CONFIG_MC_5_1_4 && out_frame.nChannel >= 10)
				frm->ch_layout = (AVChannelLayout)AV_CHANNEL_LAYOUT_5POINT1POINT4_BACK;
			else if(chconf == CHANNEL_CONFIG_MC_7_1_2 && out_frame.nChannel >= 10)
				frm->ch_layout = (AVChannelLayout)AV_CHANNEL_LAYOUT_7POINT1POINT2;
			else if(chconf == CHANNEL_CONFIG_MC_7_1_4 && out_frame.nChannel >= 12)
				frm->ch_layout = (AVChannelLayout)AV_CHANNEL_LAYOUT_7POINT1POINT4_BACK;
			else if(chconf == CHANNEL_CONFIG_HOA_ORDER3 && out_frame.nChannel == 16)
			...
			frm->ch_layout.nb_channels = frm->channels;// set correct channel, OTHERWISE will crash when 声床+对象方式没有render成2声道的情况
			frm->ch_layout.order =  AV_CHANNEL_ORDER_UNSPEC;// not check channel == mask, 源为声床+对象方式: mask not equal to channel
			avctx->ch_layout  = frm->ch_layout;//need reset avctx->ch_layout’s channles for ff_get_buffer to get correct size

			frm->format = AV_SAMPLE_FMT_S16;
			av_log(avctx, AV_LOG_DEBUG, " before ff_get_buffer! out_frame.nlen %d\n", out_frame.nlen);
			...
			if(out_frame.nlen > 0)
			{
				...
				memcpy(frm->data[0], out_frame.pOutData, out_frame.nlen);
				av_log(avctx, AV_LOG_DEBUG, "end copy buffer!\n");
				frm->buf[0] = av_buffer_create(frm->data[0], out_frame.nlen, NULL, NULL, 0);
				if (!frm->buf[0])
				{
			...
			/*if(out_frame.nlen > 0)
			{
				fwrite(out_frame.pOutData, 1, out_frame.nlen, fpout);
			}
			fclose(fpout);*/
	...
	if (h->eos){
		av_log(avctx, AV_LOG_DEBUG, "Decoder end: return AVERROR_EOF \n");
		return AVERROR_EOF;
	}
	av_log(avctx, AV_LOG_DEBUG, "Decoder end: return AVERROR(EAGAIN) \n");
    return AVERROR(EAGAIN);
}
...
{
    av_log(avctx, AV_LOG_DEBUG, "arcdav3a_decode_close!\n");
	arcdav3a_context *h = avctx->priv_data;
	if (h->m_hAvs3)
		h->avs3_destroy_decoder(h->m_hAvs3);
	h->m_hAvs3 = NULL;
	...
	pthread_cond_signal(&h->framequeue.pktcond);
	pthread_cond_signal(&h->framequeue.cond);
	pthread_join(h->thread, NULL);
	pthread_mutex_destroy(&h->framequeue.mutex);
	pthread_mutex_destroy(&h->framequeue.pktmutex);
    pthread_cond_destroy(&h->framequeue.cond);
	pthread_cond_destroy(&h->framequeue.pktcond);
    ...
    // h->t3 = millisecondsSinceEpoch;
	// av_log(avctx, AV_LOG_DEBUG, "decoed time: %lld\n", h->t2 - h->t1);
    return 0;
}
...
const FFCodec ff_libarcdav3a_decoder = 
{
	...
	.p.capabilities   = AV_CODEC_CAP_CHANNEL_CONF | AV_CODEC_CAP_DR1 | AV_CODEC_CAP_OTHER_THREADS,
	.caps_internal    = FF_CODEC_CAP_INIT_CLEANUP,
	.priv_data_size   = sizeof(arcdav3a_context),
	FF_CODEC_RECEIVE_FRAME_CB(arcdav3a_receive_frame),
